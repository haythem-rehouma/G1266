...
Avec λ = 0.3, la mise à jour se concentre surtout sur **Jour +1**, et l’influence diminue rapidement pour les jours suivants.

Schéma Mermaid (corrigé) :
...
### Exemple 1 : λ = 0.3 (réaction très locale)

Pour λ = 0.3, l’algorithme réagit surtout à ce qui se passe **le lendemain** ; les jours plus éloignés comptent de moins en moins.

```mermaid
flowchart LR
    A["Note actuelle<br>aujourd'hui<br>TD(λ = 0.3)"] --> J1["Jour +1<br>influence dominante"]
    J1 --> J2["Jour +2<br>influence modérée"]
    J2 --> J3["Jour +3<br>influence faible"]
    J3 --> J4["Jour +4<br>influence très faible"]
```
...
### Exemple 1 : λ = 0.3 (proche de TD(0))

Avec λ = 0.3, on est proche d’un comportement **TD(0)** : on regarde surtout l’avenir immédiat, en gardant une petite contribution des jours suivants.

```mermaid
flowchart LR
    A["Note actuelle<br>aujourd'hui<br>TD(λ = 0.3)"] --> J1["Jour +1<br>poids très important"]
    J1 --> J2["Jour +2<br>poids encore présent"]
    J2 --> J3["Jour +3<br>poids faible"]
    J3 --> J4["Jour +4<br>poids quasi négligeable"]
```
