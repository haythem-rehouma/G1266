...
Un gestionnaire souhaite évaluer un employé dont la performance fluctue d’un jour à l’autre : certaines journées sont nettement en dessous des attentes, d’autres sont excellentes. S’il choisit de l’évaluer uniquement sur la performance du lendemain, une seule mauvaise journée peut faire chuter fortement la note, même si les jours suivants témoignent d’un très bon niveau. À l’inverse, s’il se base uniquement sur une période longue, la note devient beaucoup plus stable, mais elle réagit très lentement aux améliorations récentes ou à une dégradation durable du travail. Le paramètre λ formalise précisément ce compromis entre réactivité et stabilité dans l’évaluation. Le problème central de TD(λ) consiste donc, pour le gestionnaire, à choisir un λ qui ne pénalise pas excessivement une contre-performance isolée, tout en reflétant correctement la tendance globale des performances de l’employé.
...
### Exemple 1 : λ = 0.3 (mémoire courte)

Quand λ = 0.3, la mise à jour dépend fortement du premier jour futur, puis l’impact décroît très vite pour les jours suivants.

```mermaid
flowchart LR
    A["Note actuelle<br>aujourd'hui<br>TD(λ = 0.3)"] --> J1["Jour +1<br>très influent"]
    J1 --> J2["Jour +2<br>peu influent"]
    J2 --> J3["Jour +3<br>influence minimale"]
    J3 --> J4["Jour +4<br>presque aucune influence"]
```
...
### Exemple 2 : λ = 0.7 (moins sensible aux anomalies)

Avec λ = 0.7, une seule mauvaise journée ne suffit plus à faire basculer la note : ce sont les tendances sur plusieurs jours qui comptent.

```mermaid
flowchart LR
    A["Note actuelle<br>aujourd'hui<br>TD(λ = 0.7)"] --> J1["Jour +1<br>très influent"]
    J1 --> J2["Jour +2<br>influence encore forte"]
    J2 --> J3["Jour +3<br>influence significative"]
    J3 --> J4["Jour +4<br>influence plus faible mais non nulle"]
```
